...
// Client-Side Canvas Image Compression Utility (<60KB WebP/JPEG) for Instant Speed
function compressImageFile(file, maxWidth = 1000, maxHeight = 1300, quality = 0.75) {
...
async function fetchProductsFromSupabase() {
  ...
  try {
    ...
    if (data && !error && data.length > 0) {
      ...
      localStorage.setItem('kaizen_products_cache', JSON.stringify(PRODUCTS_DATA));
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'kaizen_products_cache' || e.key === 'kaizen_products_updated') {
      loadInitialCachedData();
      if (path.includes('shop') || document.getElementById('shop-products-grid')) {
        initShopPage();
      } else if (path.includes('admin') || document.querySelector('.admin-layout')) {
  ...
  try {
    ...
    if (compressedDataUrl) {
      if (currentProductUploadedImages.length <= slotIndex) {
        currentProductUploadedImages[slotIndex] = compressedDataUrl;
      } else {
        currentProductUploadedImages[slotIndex] = compressedDataUrl;
      }
      // Compact array removing undefined/null holes
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        alert('An error occurred while saving product: ' + err.message);
      } finally {
        if (submitBtn) {
