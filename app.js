...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('storage', (e) => {
    if (e.key === 'kaizen_products_cache' || e.key === 'kaizen_products_updated' || e.key === 'kaizen_categories_updated') {
      loadInitialCachedData();
      if (path.includes('shop') || document.getElementById('shop-products-grid')) {
        initShopPage();
      } else if (path.includes('admin') || document.querySelector('.admin-layout')) {
...
function initProductDetailPage() {
  ...
  if (sizeContainer) {
    const standardSizes = CATEGORY_STANDARD_SIZES[product.category] || product.sizes || ["S", "M", "L", "XL", "XXL"];
    const availableSizes = Array.isArray(product.sizes) ? product.sizes : [product.sizes];

    // Pick first available size as default selection
    const firstAvailable = standardSizes.find(s => availableSizes.includes(s)) || availableSizes[0];
      ...
      try {
        ...
        if (db) {
          ...
          if (error) {
            ...
            alert('Notice: Supabase encountered an error (' + error.message + '). Saving locally.');
          }
        }
...
window.handleSingleSlotUpload = async function(event, slotIndex) {
  ...
  if (label) label.textContent = 'Uploading to Supabase...';

  try {
    const uploadedUrl = await uploadImageFileToSupabaseStorage(file);
    if (uploadedUrl) {
      if (currentProductUploadedImages.length <= slotIndex) {
        currentProductUploadedImages[slotIndex] = uploadedUrl;
      } else {
        currentProductUploadedImages[slotIndex] = uploadedUrl;
      }
      currentProductUploadedImages = currentProductUploadedImages.filter(Boolean);
      window.renderProductMultiImagesGrid();
      showToast(`Photo ${slotIndex + 1} uploaded to Supabase Storage!`);
    }
  } catch (err) {
...
window.renderProductMultiImagesGrid = function() {
  ...
  if (currentProductUploadedImages.length === 0) {
    grid.innerHTML = '';
    if (textInput) textInput.value = '';
    return;
  }
  ...
  if (textInput) {
    textInput.value = currentProductUploadedImages.join(', ');
  }

  grid.innerHTML = currentProductUploadedImages.map((imgSrc, idx) => `
    <div class="multi-image-card">
      <img src="${imgSrc}" alt="Photo ${idx + 1}" />
      ${idx === 0 ? '<span class="card-badge-main">Photo 1 (Main)</span>' : idx === 1 ? '<span class="card-badge-main" style="background:#2563eb; color:#fff;">Photo 2 (Detail)</span>' : `<span class="card-badge-main" style="background:#4b5563; color:#fff;">Photo ${idx + 1}</span>`}
      ...
      try {
        ...
        const productPayload = {
          ...
          details: {
            ...
            length: length || (category === 'kids' ? "Standard Kids Fit" : "Standard Size"),
        ...
        if (db) {
          ...
          if (error) {
            ...
            alert('Notice: Supabase sync encountered an issue (' + error.message + '). Saving locally.');
          }
        }
        ...
        renderAdminProductsTable();
        renderAdminDashboard();
        showToast(`Outfit "${code}" saved and published to website successfully!`);
      } catch (err) {
