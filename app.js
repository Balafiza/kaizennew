...
const DEFAULT_STORE_CONFIG = {
  ...
  whatsappNumber: "919500527490",
  displayPhone: "+91 9500527490",
...
// Standard Size Templates per Category for Consistency
const CATEGORY_STANDARD_SIZES = {
  kids: ["18", "20", "22", "24", "26", "28", "30", "32"],
  sarees: ["Free Size (6.3m)", "With Unstitched Blouse", "With Stitched Blouse"],
  kurtis: ["XS", "S", "M", "L", "XL", "XXL", "3XL"],
  "3piece": ["XS", "S", "M", "L", "XL", "XXL", "3XL"],
  dressmaterials: ["Unstitched (Top 2.5m, Bottom 2.5m, Dupatta 2.25m)"],
  women: ["XS", "S", "M", "L", "XL", "XXL"]
};

// Client-Side Canvas Image Compression Utility (<35KB WebP/JPEG) for Instant Speed & Zero Storage Quota Errors
function compressImageFile(file, maxWidth = 600, maxHeight = 800, quality = 0.65) {
...
async function uploadImageFileToSupabaseStorage(file) {
  if (!file) return null;
  if (!db) {
    console.warn("Supabase client not available, fallback to compressed data URL");
    return await compressImageFile(file);
  }

  try {
    // 1. Compress image to clean Blob
    const compressedDataUrl = await compressImageFile(file, 900, 1200, 0.78);
    ...
    const fileName = `outfits/${Date.now()}_${Math.random().toString(36).substring(2, 8)}.${fileExt}`;

    // 3. Upload to Supabase Storage Bucket
    const { data, error } = await db.storage.from('product-images').upload(fileName, blob, {
      cacheControl: '3600',
      upsert: true,
...
// Safe LocalStorage setter with quota fallback
function safeSetLocalStorage(key, value) {
  try {
    localStorage.setItem(key, value);
  } catch (e) {
    console.warn(`LocalStorage quota exceeded for ${key}. Clearing bloated caches.`);
    try {
      localStorage.removeItem('kaizen_products_cache');
      localStorage.removeItem('kaizen_hero_slides_cache');
      // Retry once
      localStorage.setItem(key, value);
    } catch (err) {
      console.warn("Could not save to localStorage, continuing with in-memory state.");
    }
  }
}
...
// Supabase Async Data Fetchers with Instant Local Cache Layer (0ms Instant Load)
function loadInitialCachedData() {
  try {
    const cachedConfig = localStorage.getItem('kaizen_store_config_cache');
    if (cachedConfig) STORE_CONFIG = JSON.parse(cachedConfig);

    const cachedProducts = localStorage.getItem('kaizen_products_cache');
    if (cachedProducts) PRODUCTS_DATA = JSON.parse(cachedProducts);

    const cachedCategories = localStorage.getItem('kaizen_categories_cache');
    if (cachedCategories) CATEGORIES_DATA = JSON.parse(cachedCategories);

    const cachedSlides = localStorage.getItem('kaizen_hero_slides_cache');
    if (cachedSlides) HERO_SLIDES_DATA = JSON.parse(cachedSlides);
  } catch (e) {
...
async function fetchStoreConfigFromSupabase() {
  ...
  try {
    ...
    if (data && !error) {
      ...
      localStorage.setItem('kaizen_store_config_cache', JSON.stringify(STORE_CONFIG));
...
async function fetchProductsFromSupabase() {
  ...
  try {
    ...
    if (data && !error && data.length > 0) {
      ...
      safeSetLocalStorage('kaizen_products_cache', JSON.stringify(PRODUCTS_DATA));
      return PRODUCTS_DATA;
    }
  } catch (err) {
    console.warn("Using cached/default products:", err);
  }
  return PRODUCTS_DATA;
}
...
async function fetchCategoriesFromSupabase() {
  ...
  try {
    ...
    if (data && !error && data.length > 0) {
      ...
      localStorage.setItem('kaizen_categories_cache', JSON.stringify(CATEGORIES_DATA));
      return CATEGORIES_DATA;
    }
  } catch (err) {
    console.warn("Using cached/default categories:", err);
  }
  return CATEGORIES_DATA;
}
  ...
  try {
    ...
    if (data && !error && data.length > 0) {
      HERO_SLIDES_DATA = data.map(h => ({
        ...
        title: h.title,
        subtitle: h.subtitle,
        btnText: h.btn_text || h.btnText,
        btnLink: h.btn_link || h.btnLink,
        image: h.image
      }));
      localStorage.setItem('kaizen_hero_slides_cache', JSON.stringify(HERO_SLIDES_DATA));
      return HERO_SLIDES_DATA;
    }
  } catch (err) {
    console.warn("Using cached/default hero slides:", err);
  }
  return HERO_SLIDES_DATA;
}

// Unified Active Page Refresh Function
function refreshActivePageView() {
  const path = window.location.pathname.toLowerCase();
  if (path.includes('shop') || document.getElementById('shop-products-grid')) {
    initShopPage();
  } else if (path.includes('product-detail') || document.getElementById('detail-main-img')) {
    initProductDetailPage();
  } else if (path.includes('admin') || document.querySelector('.admin-layout')) {
    populateCategoryDropdowns && populateCategoryDropdowns();
    renderAdminCategories && renderAdminCategories();
    renderAdminProductsTable && renderAdminProductsTable();
    renderAdminDashboard && renderAdminDashboard();
    renderAdminHeroTable && renderAdminHeroTable();
  } else if (document.getElementById('home-products-grid')) {
    renderHomeLatestProducts && renderHomeLatestProducts();
    renderHomeCategoriesCarousel && renderHomeCategoriesCarousel();
  }
}

// Supabase Real-Time WebSocket Listeners for Instant 0ms Sync across All Tabs & Devices
function setupSupabaseRealtimeSubscriptions() {
  if (!db) return;
  try {
    db.channel('kaizen-realtime-channel')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'hero_slides' }, async () => {
        await fetchHeroSlidesFromSupabase();
        refreshActivePageView();
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'store_config' }, async () => {
...
// Document Ready (Instant Render + Background Revalidation + Realtime Sync)
document.addEventListener('DOMContentLoaded', () => {
  // 1. Instant Cache Render (<10ms)
  loadInitialCachedData();
  applyStoreConfigToDOM();
  updateCartBadge();
  setupGlobalCartDrawer();
  
  // Page Specific Inits Immediately
  const path = window.location.pathname.toLowerCase();
  
  if (path.includes('admin') || document.getElementById('admin-lock-screen') || document.querySelector('.admin-layout')) {
    initAdminPage();
  } else if (path.includes('shop') || document.getElementById('shop-products-grid')) {
    initShopPage();
  } else if (path.includes('product-detail') || document.getElementById('detail-main-img')) {
    initProductDetailPage();
  } else if (path.includes('contact') || document.getElementById('contact-inquiry-form')) {
    initContactPage();
  } else {
  ...
  // 2. Background Revalidation (Fresh Data from Supabase without blocking UI)
  Promise.all([
    ...
    // Refresh active page with latest data
    refreshActivePageView();
  });

  // 3. Setup Supabase Realtime WebSocket Connection
  setupSupabaseRealtimeSubscriptions();

  // 4. Multi-Tab LocalStorage Real-time Auto-Sync Listener
  window.addEventListener('storage', (e) => {
    if (e.key === 'kaizen_products_cache' || e.key === 'kaizen_products_updated' || e.key === 'kaizen_categories_updated' || e.key === 'kaizen_categories_cache' || e.key === 'kaizen_store_config_cache') {
      loadInitialCachedData();
      applyStoreConfigToDOM();
      refreshActivePageView();
    }
...
function initHomePage() {
  ...
  // Dynamically render Home Latest & Trending Products Grid (Consistently from Supabase)
...
function renderHomeLatestProducts() {
  ...
  const products = PRODUCTS_DATA.length > 0 ? PRODUCTS_DATA : DEFAULT_PRODUCTS;
  const latestOutfits = products.slice(0, 8); // Display latest 8 outfits
  renderProductCards(latestOutfits, container);
}
...
function initShopPage() {
  ...
  function updateShopView() {
    let filtered = PRODUCTS_DATA.filter(product => {
      const matchCat = (activeCategory === 'all') || 
                       (activeCategory === 'offers' && (product.discount || product.discount_percent)) ||
      ...
      const matchSearch = searchQuery === '' || 
                          ...
                          (product.category_label && product.category_label.toLowerCase().includes(searchQuery));
    ...
    // Update active state on sidebar buttons without recreating HTML
    const sidebarButtons = document.querySelectorAll('.sidebar-category-btn');
  ...
  // Initial Sidebar Render and Event Attachment
...
async function initProductDetailPage() {
  ...
  // 1. Try to find in in-memory / cached products
  if (productId) {
    product = PRODUCTS_DATA.find(p => p.id === productId || p.code === productId);
  }

  // 2. If not found in cache and db exists, fetch single product directly from Supabase
  if (productId && !product && db) {
    try {
      const { data, error } = await db.from('products').select('*').or(`id.eq.${productId},code.eq.${productId}`).maybeSingle();
      if (data && !error) {
        product = {
          id: data.id,
          code: data.code,
          ...
          rating: data.rating,
          reviewsCount: data.reviews_count || data.reviewsCount,
        ...
        // Add to local cache so subsequent lookups are instant
        const existingIdx = PRODUCTS_DATA.findIndex(p => p.id === product.id);
        if (existingIdx >= 0) {
          PRODUCTS_DATA[existingIdx] = product;
        } else {
          PRODUCTS_DATA.unshift(product);
        }
        safeSetLocalStorage('kaizen_products_cache', JSON.stringify(PRODUCTS_DATA));
      }
    } catch (e) {
      console.warn("Direct Supabase product fetch error:", e);
    }
  }

  // 3. Fallback: If no productId was in URL at all, load first available product
  if (!productId && !product) {
    product = PRODUCTS_DATA[0] || DEFAULT_PRODUCTS[0];
  }

  // 4. If productId was given but product truly does not exist in DB or cache
  if (productId && !product) {
    const detailSection = document.querySelector('.product-detail-section .container');
    if (detailSection) {
      detailSection.innerHTML = `
        <div class="breadcrumbs" style="margin-bottom: 24px;">
          <a href="index.html">Home</a> <span>/</span> <a href="shop.html">Shop Collection</a> <span>/</span> <span>Product Not Found</span>
        </div>
        <div style="text-align: center; padding: 60px 20px; color: var(--text-muted); background: #121218; border: 1px solid var(--gold-border); border-radius: var(--radius-md);">
          <i class="fa-solid fa-triangle-exclamation" style="font-size: 3.5rem; color: var(--gold-primary); margin-bottom: 18px;"></i>
          <h2 style="color: var(--gold-light); font-size: 1.6rem; margin-bottom: 10px;">Outfit Not Found</h2>
          <p style="font-size: 0.95rem; max-width: 480px; margin: 0 auto 24px;">The outfit you are looking for (ID: <code>${productId}</code>) is unavailable or may have been updated in the catalog.</p>
          <a href="shop.html" class="btn-primary-gold" style="display: inline-flex; padding: 12px 28px; font-size: 0.9rem;">
            <i class="fa-solid fa-bag-shopping"></i> Explore All Outfits
          </a>
        </div>
      `;
    }
  ...
  const productSizes = Array.isArray(product.sizes) && product.sizes.length > 0 ? product.sizes : ["Free Size"];
  const productColors = Array.isArray(product.colors) && product.colors.length > 0 ? product.colors : [{ name: "Standard", hex: "#d4af37", img: (product.images && product.images[0]) || 'images/hero_saree.jpg' }];
  const productImages = Array.isArray(product.images) && product.images.length > 0 ? product.images : ['images/hero_saree.jpg'];
  const productDetails = product.details || { fabric: "Premium Fabric", length: "Standard", work: "Handcrafted", occasion: "Festive & Party Wear", care: "Dry Clean / Gentle Wash" };
  ...
  if (breadcrumbEl) {
    breadcrumbEl.innerHTML = `<a href="index.html">Home</a> <span>/</span> <a href="shop.html?category=${product.category || 'all'}">${product.categoryLabel || product.category_label || 'Shop'}</a> <span>/</span> <strong style="color: var(--gold-light);">${product.title || 'Outfit'}</strong>`;
  }

  // Main Image
  const mainImgEl = document.getElementById('detail-main-img');
  if (mainImgEl) mainImgEl.src = productImages[0];
  ...
  if (thumbsContainer) {
    thumbsContainer.innerHTML = productImages.map((img, i) => `
      <div class="detail-thumb ${i === 0 ? 'active' : ''}" onclick="changeDetailImage('${img}', this)">
        <img src="${img}" alt="${product.title || 'Outfit'}" onerror="this.src='images/hero_saree.jpg'" />
      </div>
    `).join('');
  }

  // Titles & Codes
  setText('detail-title', product.title || 'Ethnic Outfit');
  setText('detail-code', `Product Code: ${product.code || product.id}`);
  const priceVal = product.price ? parseInt(product.price, 10) : 0;
  const origPriceVal = (product.originalPrice || product.original_price) ? parseInt(product.originalPrice || product.original_price, 10) : priceVal;
  setText('detail-price', `${STORE_CONFIG.currency}${priceVal.toLocaleString('en-IN')}`);
  setText('detail-original-price', origPriceVal > priceVal ? `${STORE_CONFIG.currency}${origPriceVal.toLocaleString('en-IN')}` : '');
  setText('detail-discount', product.discount ? `(${product.discount})` : '');
  setText('detail-description', product.description || 'Exclusive designer collection crafted with premium materials.');

  // Reviews
  const reviewsEl = document.getElementById('detail-reviews');
  if (reviewsEl) {
    reviewsEl.innerHTML = `${renderStars(product.rating || 5.0)} <span>(${product.reviewsCount || product.reviews_count || 1} verified reviews)</span>`;
  }

  // Sizes (Render all standard sizes for the category, mark unavailable/out-of-stock sizes as struck-out/disabled)
  const sizeContainer = document.getElementById('detail-size-container');
  if (sizeContainer) {
    const standardSizes = CATEGORY_STANDARD_SIZES[product.category] || productSizes;
    const firstAvailable = standardSizes.find(s => productSizes.includes(s)) || productSizes[0] || standardSizes[0];
    selectedSize = firstAvailable;

    sizeContainer.innerHTML = standardSizes.map((s) => {
      const isAvailable = productSizes.includes(s) || standardSizes.length === 1;
      if (isAvailable) {
        const isSelected = s === firstAvailable;
        return `<button class="size-btn ${isSelected ? 'active' : ''}" onclick="selectDetailSize('${s}', this)" title="In Stock: Size ${s}">${s}</button>`;
      } else {
        return `<button class="size-btn disabled" title="Size ${s} - Currently Out of Stock" aria-disabled="true">${s}</button>`;
      }
    }).join('');
  }
  ...
  if (colorContainer) {
    colorContainer.innerHTML = productColors.map((c, i) => `
      <button class="color-swatch ${i === 0 ? 'active' : ''}" style="background-color: ${c.hex || '#d4af37'};" title="${c.name || 'Color'}" onclick="selectDetailColor('${c.name || 'Standard'}', '${c.img || productImages[0]}', this)"></button>
    `).join('');
  }

  // Specs
  setText('spec-fabric', productDetails.fabric || 'Premium Quality');
  setText('spec-length', productDetails.length || 'Standard Dimensions');
  setText('spec-work', productDetails.work || 'Handcrafted Detailing');
  setText('spec-occasion', productDetails.occasion || 'Festive, Wedding & Party Wear');
  setText('spec-care', productDetails.care || 'Dry Clean / Gentle Hand Wash');
...
function renderProductCards(products, container) {
  ...
  container.innerHTML = products.map(product => {
    const imgSrc = (Array.isArray(product.images) && product.images[0]) || product.image || 'images/hero_saree.jpg';
    const prodTitle = product.title || product.name || 'Ethnic Outfit';
    const catLabel = product.categoryLabel || product.category_label || (product.category ? product.category.toUpperCase() : 'Collection');
    const prodPrice = product.price ? parseInt(product.price, 10) : 0;
    const origPrice = (product.originalPrice || product.original_price) ? parseInt(product.originalPrice || product.original_price, 10) : prodPrice;
    const discountText = product.discount || (origPrice > prodPrice ? `${Math.round(((origPrice - prodPrice) / origPrice) * 100)}% OFF` : '');
    const reviewsCount = product.reviewsCount || product.reviews_count || 1;
    const ratingVal = product.rating || 5.0;

    return `
      <div class="product-card" data-id="${product.id}">
        <a href="product-detail.html?id=${product.id}" class="product-image-wrap">
          <img src="${imgSrc}" alt="${prodTitle}" loading="lazy" onerror="this.src='images/hero_saree.jpg'" />
          ${discountText ? `<span class="badge-discount-tag">${discountText}</span>` : ''}
          <span class="badge-stock-tag">In Stock</span>
        </a>
        <div class="product-info">
          <div class="product-category-text">${catLabel}</div>
          <a href="product-detail.html?id=${product.id}" class="product-title">${prodTitle}</a>
          <div class="product-sku">${product.code || product.id}</div>
          <div class="product-rating">
            ${renderStars(ratingVal)}
            <span>(${reviewsCount})</span>
          </div>
          <div class="product-price-row">
            <span class="price-current">${STORE_CONFIG.currency}${prodPrice.toLocaleString('en-IN')}</span>
            ${origPrice > prodPrice ? `<span class="price-original">${STORE_CONFIG.currency}${origPrice.toLocaleString('en-IN')}</span>` : ''}
            ${discountText ? `<span class="price-discount-percent">(${discountText})</span>` : ''}
          </div>
...
function directWhatsAppOrder(productId) {
  ...
  const defaultSize = (Array.isArray(product.sizes) && product.sizes[0]) || 'Standard';
  const defaultColor = (Array.isArray(product.colors) && product.colors[0]?.name) || 'Standard';
  const categoryLabel = product.categoryLabel || product.category_label || (product.category ? product.category.toUpperCase() : 'Collection');
  const qty = 1;
  const prodPrice = product.price ? parseInt(product.price, 10) : 0;
...
🛍️ *Product:* ${product.title || 'Outfit'}
🔖 *Product Code:* ${product.code || product.id}
📁 *Category:* ${categoryLabel}
💰 *Price:* ${STORE_CONFIG.currency}${prodPrice.toLocaleString('en-IN')}
📏 *Size:* ${defaultSize}
🎨 *Color:* ${defaultColor}
🔢 *Quantity:* ${qty}
...
function orderFromDetailWhatsApp() {
  ...
  const color = selectedColor?.name || (Array.isArray(product.colors) && product.colors[0]?.name) || 'Standard';
  const categoryLabel = product.categoryLabel || product.category_label || (product.category ? product.category.toUpperCase() : 'Collection');
...
🔖 *Product Code:* ${product.code || product.id}
📁 *Category:* ${categoryLabel}
💰 *Unit Price:* ${STORE_CONFIG.currency}${prodPrice.toLocaleString('en-IN')}
📏 *Size:* ${size}
🎨 *Color:* ${color}
🔢 *Quantity:* ${qty}
...
function addDetailToCart() {
  ...
  const color = selectedColor?.name || (Array.isArray(product.colors) && product.colors[0]?.name) || 'Standard';
  const categoryLabel = product.categoryLabel || product.category_label || (product.category ? product.category.toUpperCase() : 'Collection');
  const image = (Array.isArray(product.images) && product.images[0]) || product.image || 'images/hero_saree.jpg';
  const prodPrice = product.price ? parseInt(product.price, 10) : 0;

  const cartItem = {
    cartId: `${product.id}_${size}_${color}`,
    id: product.id,
    title: product.title || 'Outfit',
...
function setupAdminCategoryActions() {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      const originalBtnHtml = submitBtn ? submitBtn.innerHTML : '<i class="fa-solid fa-check"></i> SAVE CATEGORY';
      ...
      try {
        ...
        const slugId = (rawId || name).toLowerCase().replace(/[^a-z0-9_-]/g, '-') || 'cat_' + Date.now().toString().slice(-4);
        ...
        const image = document.getElementById('modal-category-image').value.trim() || "images/cat_saree.jpg";
        ...
        if (db) {
          ...
          if (error) {
            ...
            showToast('Note: Saved locally. Supabase error: ' + error.message);
          }
        }

        // 1. Optimistic in-memory update
        const existingIdx = CATEGORIES_DATA.findIndex(c => c.id === slugId);
        if (existingIdx >= 0) {
          CATEGORIES_DATA[existingIdx] = categoryObj;
        } else {
          CATEGORIES_DATA.push(categoryObj);
        }
        safeSetLocalStorage('kaizen_categories_cache', JSON.stringify(CATEGORIES_DATA));

        // 2. Broadcast change
        try {
          localStorage.setItem('kaizen_categories_updated', Date.now().toString());
        } catch (e) {}
        ...
        alert('An error occurred while saving category: ' + err.message);
      } finally {
        if (submitBtn) {
  ...
  // Auto-generate slug ID as user types category name
  const catNameInput = document.getElementById('modal-category-name');
  const catIdInput = document.getElementById('modal-category-id');
  if (catNameInput && catIdInput) {
    catNameInput.addEventListener('input', () => {
      const isNew = document.getElementById('modal-category-is-new') && document.getElementById('modal-category-is-new').value === 'true';
      if (isNew) {
        catIdInput.value = catNameInput.value
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/^-+|-+$/g, '');
      }
...
window.deleteCategoryAdmin = async function(categoryId) {
  ...
  if (confirm(confirmMsg)) {
    ...
    CATEGORIES_DATA = CATEGORIES_DATA.filter(c => c.id !== categoryId);
    safeSetLocalStorage('kaizen_categories_cache', JSON.stringify(CATEGORIES_DATA));
    try {
      localStorage.setItem('kaizen_categories_updated', Date.now().toString());
    } catch (e) {}
...
window.toggleProductStockAdmin = async function(id) {
  ...
  if (product) {
    ...
    safeSetLocalStorage('kaizen_products_cache', JSON.stringify(PRODUCTS_DATA));
    try {
      localStorage.setItem('kaizen_products_updated', Date.now().toString());
    } catch (e) {}
    renderAdminProductsTable();
    ...
    if (db) {
      ...
      if (error) {
        console.warn('Error updating stock in Supabase:', error);
      }
    }
  }
};
...
window.deleteProductAdmin = async function(id) {
  if (confirm('Are you sure you want to delete this product from your catalog?')) {
    ...
    renderAdminDashboard();
    fetchProductsFromSupabase();
    showToast('Product deleted from Supabase DB successfully.');
  }
};
...
// Multi-Image Upload State for Admin Product Modal
let currentProductUploadedImages = [];

window.clearProductImageUpload = function() {
  currentProductUploadedImages = [];
  ['modal-product-file-1', 'modal-product-file-2', 'modal-product-file-3'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  const textInput = document.getElementById('modal-product-image');
  const grid = document.getElementById('product-multi-images-grid');

  if (textInput) textInput.value = '';
  if (grid) grid.innerHTML = '';
};

window.handleSingleSlotUpload = async function(event, slotIndex) {
  const file = event.target.files[0];
  if (!file) return;
  
  const label = event.target.parentElement ? event.target.parentElement.querySelector('.upload-dropzone-content span') : null;
  const originalText = label ? label.textContent : '';
  if (label) label.textContent = 'Uploading...';

  try {
    const uploadedUrl = await uploadImageFileToSupabaseStorage(file);
    if (uploadedUrl) {
      currentProductUploadedImages[slotIndex] = uploadedUrl;
      window.renderProductMultiImagesGrid();
      showToast(`Photo ${slotIndex + 1} uploaded successfully!`);
    }
  } catch (err) {
    console.warn("Slot upload error:", err);
    showToast('Upload error: ' + err.message);
  } finally {
    if (label) label.textContent = originalText || `Photo ${slotIndex + 1}`;
  }
};

window.handleMultipleSlotUpload = async function(event) {
  const files = Array.from(event.target.files);
  if (!files || files.length === 0) return;
  showToast(`Uploading ${files.length} photo(s)...`);
  for (const file of files) {
    const url = await uploadImageFileToSupabaseStorage(file);
    if (url) {
      currentProductUploadedImages.push(url);
    }
  }
  window.renderProductMultiImagesGrid();
...
window.renderProductMultiImagesGrid = function() {
  const grid = document.getElementById('product-multi-images-grid');
  const textInput = document.getElementById('modal-product-image');
  if (!grid) return;

  const validImages = currentProductUploadedImages.filter(Boolean);
  currentProductUploadedImages = validImages;
  ...
  if (textInput) {
    textInput.value = validImages.join(', ');
  }

  grid.innerHTML = validImages.map((imgSrc, idx) => `
    <div class="multi-image-card">
      <img src="${imgSrc}" alt="Photo ${idx + 1}" onerror="this.src='images/hero_saree.jpg'" />
      <span class="card-badge-main" style="${idx === 0 ? 'background:var(--gold-primary); color:#000;' : idx === 1 ? 'background:#2563eb; color:#fff;' : 'background:#4b5563; color:#fff;'}">
        ${idx === 0 ? 'Photo 1 (Main)' : idx === 1 ? 'Photo 2 (Detail)' : `Photo ${idx + 1}`}
      </span>
      <button type="button" class="btn-remove-img" onclick="removeProductUploadedImage(${idx})" title="Remove this photo">
...
window.removeProductUploadedImage = function(index) {
  currentProductUploadedImages.splice(index, 1);
  window.renderProductMultiImagesGrid();
};
...
function setupImageUploadHandlers() {
  ...
  if (prodTextInput) {
    prodTextInput.addEventListener('input', (e) => {
      ...
      if (raw) {
        currentProductUploadedImages = raw.split(',').map(s => s.trim()).filter(Boolean);
      } else {
        currentProductUploadedImages = [];
      }
      window.renderProductMultiImagesGrid();
  ...
  if (catFileInput) {
    catFileInput.addEventListener('change', async (e) => {
      ...
      if (file) {
        showToast('Uploading category image to Supabase Storage...');
        const uploadedUrl = await uploadImageFileToSupabaseStorage(file);
        if (uploadedUrl) {
          if (catPreviewImg) catPreviewImg.src = uploadedUrl;
          if (catPreviewBox) catPreviewBox.style.display = 'block';
          if (catTextInput) catTextInput.value = uploadedUrl;
          showToast('Category image uploaded to Supabase Storage!');
        }
      }
    });
  }
  ...
  if (heroFileInput) {
    heroFileInput.addEventListener('change', async (e) => {
      const file = e.target.files[0];
      if (file) {
        showToast('Uploading hero banner to Supabase Storage...');
        const uploadedUrl = await uploadImageFileToSupabaseStorage(file);
        if (uploadedUrl) {
          if (heroPreviewImg) heroPreviewImg.src = uploadedUrl;
          if (heroPreviewBox) heroPreviewBox.style.display = 'block';
          if (heroTextInput) heroTextInput.value = uploadedUrl;
          showToast('Hero banner uploaded to Supabase Storage!');
        }
      }
    });
  }
...
window.openEditProductModal = function(id) {
  ...
  const productImages = Array.isArray(product.images) && product.images.length > 0 ? product.images : ['images/hero_saree.jpg'];
  currentProductUploadedImages = [...productImages];
  window.renderProductMultiImagesGrid();
...
function setupAdminProductActions() {
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const submitBtn = form.querySelector('button[type="submit"]');
      const originalBtnHtml = submitBtn ? submitBtn.innerHTML : '<i class="fa-solid fa-check"></i> SAVE OUTFIT';
      ...
      try {
        ...
        const id = document.getElementById('modal-product-id').value.trim() || 'kp' + Date.now().toString().slice(-4);
        const title = document.getElementById('modal-product-title').value.trim();
        let code = document.getElementById('modal-product-code').value.trim();
        if (!code) {
          code = 'KH-' + Date.now().toString().slice(-4).toUpperCase();
        }
        ...
        const origPriceInput = document.getElementById('modal-product-original-price').value.trim();
        const origPrice = origPriceInput ? (parseInt(origPriceInput, 10) || price) : price;
        const badge = document.getElementById('modal-product-badge').value.trim() || "New Arrival";
        const desc = document.getElementById('modal-product-desc').value.trim() || `${title} - Exclusive collection crafted with premium quality.`;
        const fabric = document.getElementById('modal-product-fabric').value.trim() || "Premium Quality Fabric";
        const length = document.getElementById('modal-product-length').value.trim() || (category === 'kids' ? "Standard Kids Fit" : "Standard Size");
        const work = document.getElementById('modal-product-work').value.trim() || "Handcrafted Detailing";

        // Collect Multi-Images Array
        let imagesArray = currentProductUploadedImages.filter(Boolean);
        if (imagesArray.length === 0) {
          const textVal = document.getElementById('modal-product-image').value.trim();
          if (textVal) {
            imagesArray = textVal.split(/[,;\n]+/).map(s => s.trim()).filter(Boolean);
          }
        }
        if (imagesArray.length === 0) {
          imagesArray = ['images/hero_saree.jpg'];
        }

        const matchedCat = CATEGORIES_DATA.find(c => c.id === category);
        const categoryLabel = matchedCat ? matchedCat.name : (category.charAt(0).toUpperCase() + category.slice(1));

        // Assign Category Standard Sizes (Kids: 18-32, Sarees: Free Size, etc.)
        const standardSizes = CATEGORY_STANDARD_SIZES[category] || ["S", "M", "L", "XL", "XXL"];
        const discountText = origPrice > price ? `${Math.round(((origPrice - price) / origPrice) * 100)}% OFF` : '';
        ...
        const productPayload = {
          ...
          sizes: standardSizes,
          colors: [{ name: "Standard Color", hex: "#d4af37", img: imagesArray[0] }],
          images: imagesArray
        };

        if (db) {
          // Upsert to DB with clean snake_case columns
          const dbPayload = {
            id: productPayload.id,
            code: productPayload.code,
            title: productPayload.title,
            category: productPayload.category,
            category_label: productPayload.categoryLabel,
            price: productPayload.price,
            original_price: productPayload.originalPrice,
            discount: productPayload.discount,
            rating: productPayload.rating,
            reviews_count: productPayload.reviewsCount,
          ...
          const { error } = await db.from('products').upsert([dbPayload]);
          if (error) {
        ...
        // 1. Immediately update in-memory and local cache
        const existingIdx = PRODUCTS_DATA.findIndex(p => p.id === id);
        if (existingIdx >= 0) {
          PRODUCTS_DATA[existingIdx] = productPayload;
        } else {
          PRODUCTS_DATA.unshift(productPayload);
        }
        safeSetLocalStorage('kaizen_products_cache', JSON.stringify(PRODUCTS_DATA));

        // 2. Broadcast change to all open tabs / pages
        try {
          localStorage.setItem('kaizen_products_updated', Date.now().toString());
        } catch (e) {}

        // 3. Update all Admin Views
        closeAdminModal();
        renderAdminProductsTable();
        renderAdminDashboard();
        renderAdminCategories();
        showToast(`Outfit "${code}" saved and published successfully!`);
      } catch (err) {
