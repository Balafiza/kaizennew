...
// Document Ready (Instant Render + Background Revalidation)
document.addEventListener('DOMContentLoaded', () => {
  ...
  Promise.all([
    ...
    // Seamlessly refresh active page data in background
  ...
  // 3. Multi-Tab Real-time Auto-Sync Listener
  window.addEventListener('storage', (e) => {
    if (e.key === 'kaizen_products_cache' || e.key === 'kaizen_products_updated' || e.key === 'kaizen_categories_updated' || e.key === 'kaizen_categories_cache') {
      loadInitialCachedData();
      if (path.includes('shop') || document.getElementById('shop-products-grid')) {
        initShopPage();
      } else if (path.includes('admin') || document.querySelector('.admin-layout')) {
...
function initProductDetailPage() {
  const urlParams = new URLSearchParams(window.location.search);
  const productId = urlParams.get('id') || (PRODUCTS_DATA[0] && PRODUCTS_DATA[0].id) || 'ks102';
  const product = PRODUCTS_DATA.find(p => p.id === productId) || PRODUCTS_DATA[0] || DEFAULT_PRODUCTS[0];
